...
export function sketch() {
    ...
    function updateCubes(object, time, numblobs, s, floor, wallx, wallz) {
        ...
        for (let x = r; x <= numblobs; x++) {
                const ballx = x * .1 + (Math.sin(time))
                console.log(ballx)
                const bally = x * .1 + (Math.cos(time)) // + (Math.random() * sy * time)
                console.log(bally)
                const ballz = r // + (Math.random() * sz * time) // Math.cos(z + 1.32 * time * 0.1 * Math.sin((0.92 + 0.53 * z))) * 0.27 + 0.5 // z + sz
    ...
    const animate = () => {
       ...
       /*  effectController.sx = effectController.sx + noise3D(0, t, 0) * .2
        effectController.sy = effectController.sy + noise3D(0, t, 0) * .1
        effectController.sz = effectController.sz + noise3D(0, t, 0) * .2 */
